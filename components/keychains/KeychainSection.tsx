...
import KeychainCard, { KEYCHAIN_PRICE } from "./KeychainCard";
import { add, dram } from "@/lib/cart";
import { flyToCart } from "@/lib/fly";
import type { Keychain, KeychainWall } from "@/types/keychain";

// ============================================================================
// THE KEYCHAIN SHOWROOM — the whole line on one endless wall, on paper.
//
// Fifth pass (client, change.pdf p6, 2026-09-21, with a screenshot of the
// hooked keychains from the 1 September site: «these previous hangers looked
// better, neutral — can't we bring them back, but plain, with a light
// background? this colour», pointing at the page's cream). So this is the
// third pass again — the carousel of 2026-08-31 — restored from this file's
// own history, and the walnut board that replaced it on 2026-09-01 is gone
// (HangingKeychainDisplay.tsx deleted with its wood photographs). What
// changed since that carousel last shipped is only the ground: the dark room
// became the page's paper on 2026-09-15 (keychains.css), and the hooks, the
// arrows and the pill are re-coloured for it.
//
...
//
// THE PRICE IS READ, NOT TYPED: KEYCHAIN_PRICE comes from `categories` in
// lib/content.ts (1,500 ֏ on her 2026-09-21 list), the same number the cart
// and /api/order use.
// ============================================================================
...
export default function KeychainSection({
  ...
  return (
    <section className="ap-kc" data-live={live || undefined} aria-labelledby="ap-kc-title">
      {/* the room — the page's own paper now. Purely decorative, so it is
          drawn in CSS and announced to nobody. */}
      ...
      <div className="ap-kc__head">
        ...
        <p className="ap-kc__from">
          <strong>{dram(KEYCHAIN_PRICE)}</strong> each
        </p>
      </div>
